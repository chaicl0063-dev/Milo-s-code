...
## 24. GPT Work QA · 2026-09-15（官网 v3 第一轮验收 · company）

**结论：发现 2 个 Blocking Issues，修复后复验；设计方向保持冻结。**

审核基线：实现提交 `3ef423a`，当前 HEAD `6e708fc`（仅补交接记录），`company` 工作区在检查前干净。仓库中的 implementation spec 与 GPT Work 输出原件 SHA-256 一致。

本轮已独立检查：实际代码 diff、桌面 1280 首视口与 1440 整页/关键区块截图、手机 375 首视口与整页截图（逐段查看，并按 2× 截图还原到 375px 阅读尺寸）、路线素材及对应 fixture。禁止修改的 App 全局样式、页面逻辑、API、Tab 与路线存储未出现在本轮 diff 中。

### BI-01 · S05 Keep Exploring：手机路线信息不可读

- **证据**：`docs/screens/mobile-full-375.jpg` 的 Keep Exploring 区块；`components/site/SiteLanding.tsx:223` 的展示容器无条件使用 `grid-cols-2`。原图为 750px 宽的 2× 截图，不能放大到 750px 后当作手机上的可读尺寸。
- **表现**：375px 手机上，扣除页面边距、面板内边距和间隙后，每张真实 UI 截图约宽 145px。原 390px 宽 App 界面被缩成约 37%，站点名称与站间说明落到约 5—7px，用户难以读清路线。没有横向溢出不代表展示可用。
- **为何阻断**：该屏必须让用户看懂编号地点和路线顺序；冻结 spec §3 S05 / §8.3 明确要求真实路线可读，不能用微缩截图替代。
- **最小修复**：手机改为单列，地图在前、同一路线结果在后；双列只在实际可读的宽屏启用。地图按需裁掉与路线无关的空白，结果保留真实地点名、站号、站间时间与总时长。只改官网展示容器和截图 framing，不重画 UI、不改路线数据、不改 App 逻辑。
- **复测条件**：提交 375px 的 S05 局部原尺寸截图，并检查 360/430px；正常缩放下可读地点名、站号与路线摘要，无需双指放大。若仅改为单列后仍小于约 12px 的关键信息，应进一步裁切无关边缘以放大真实内容。1440px 桌面顺序和可读性不退化。

### BI-02 · S08 Free / Plus：手机邮箱输入框高度被压缩

- **证据**：同一手机整页截图中的 Plus 邮箱表单；`components/site/SiteLanding.tsx:324` 为 `flex-col ... sm:flex-row`，`:332` 的输入框同时设 `h-11` 与 `flex-1`。
- **表现**：按 375px 实际显示尺寸还原，输入框外框高度约 22px，明显低于下方约 44px 的 Get notified 按钮；占位文字几乎贴边，触摸输入目标过窄。源码写有 h-11 不能替代实际渲染结果。
- **为何阻断**：这是现有可操作表单，不是装饰图；冻结 spec 的手机控件要求和可用性验收未满足。
- **最小修复**：给输入框明确至少 44px 的最小高度，手机纵向布局不使用 `flex-1` 控制其主轴高度，例如 `h-11 min-h-11 flex-none sm:flex-1`；横排时再保留宽度伸展。核对实际 computed height，保证不会被 flex 计算压缩。邮箱校验、提交接口与成功/失败逻辑保持不变。
- **复测条件**：提供 375px 的 S08 局部截图与输入框 `getBoundingClientRect().height` 实测（≥44 CSS px）；检查 360/430px 和桌面横排。实际点击输入、键盘聚焦、输入文字均正常，无裁切，原提交状态不受影响。

### 第 23 节已提出偏差的裁决

- **Hero 两块左右并排：接受，不构成 Blocking。** 桌面首视口中产品定义、地图、地点名、开讲入口与 Mia 可见；手机按单列展示，地图与地点名进入首屏。不要求为了机械遵守上下比例返工。
- **See 桌面留白：接受，不构成 Blocking。** 核心地图与地点卡可读，未遮挡信息；不增加填充文案、图片或新模块。

### 验证边界与下一步

本次是对 Claude 第一轮截图、代码及交接证据的审核，未重新操作实时浏览器或重复运行完整 build/test。第 23 节的 lint / tsc / 25 条测试 / build 与音频操作结果属于 Claude 提供的记录，不冒称 GPT Work 独立复测。此前标注未测的音频 error 态、真机 iframe 权限等继续保留未测状态；本次未发现证据表明它们因视觉改版损坏，不额外增加新的 Blocking Issue。

Claude 下一步**只修 BI-01 / BI-02**，优先限制在 `components/site/SiteLanding.tsx`，不重开 Hero 或 See，不增加功能。交回修复提交、两处手机局部截图、桌面回归截图和输入框实际高度；若只改相关布局，不要求再生成一条路线或重新拍全部 App 源截图。GPT Work 针对两项复验，通过后沿用既定运维交付流程。本次不修改源码、不部署、不改 main / personal。

## 25. Claude 交接 · 2026-09-15（第十轮：只修 BI-01 / BI-02 · `company`）

依据第 24 节。只改 `components/site/SiteLanding.tsx` 两处，外加截图脚本补两张手机局部图和输入框高度测量。没有动 Hero、See、路线数据、应用逻辑；没有部署，没有动 `main` / `personal`。

### 修复

- **BI-01 · S05 路线板**：展示容器从无条件 `grid-cols-2` 改为 `grid-cols-1 md:grid-cols-2`。手机上地图在前、同一路线的结果在后，各占板内全宽；≥768px 才双列，桌面 1440 版式不变。截图本身没有重拍，也没有再裁（当前裁切下，地图 780/1560 去掉底部 tab，结果图 780/1247 就是整条结果消息）。
- **BI-02 · S08 邮箱输入框**：`h-11 flex-1` 改为 `h-11 min-h-11 flex-none sm:flex-1`。竖排时不再参与主轴分配，横排（≥640px）仍伸展宽度。校验、接口、状态逻辑未改。

### 复测结果（本机 dev，`node scripts/shoot-app.mjs --site` 实测 + 浏览器实际操作）

| 宽度 | S05 列数 | 路线截图显示宽度（相对应用原生 390px） | 邮箱输入框高度 | 横向溢出 |
|---|---|---|---|---|
| 360 | 1 | 292px（0.75×） | 44px | 无 |
| 375 | 1 | 307px（0.79×） | 44px（Get notified 也是 44px） | 无 |
| 430 | 1 | 362px（0.93×） | 44px | 无 |
| 1440 | 2 | 与第一轮相同 | 横排 | 无 |

- 375 下路线结果图里站名约 15px×0.79 ≈ 12px，站号、「Start · 116 m」「Walk 1 min · 17 m」「3 stops · about 1 h」肉眼可读（见 `docs/screens/mobile-section-explore.jpg`，2× 原尺寸）。地图图里路线栏「Today's route · 3 stops · about 1 h」、编号 1/2/3、当前站卡片都完整。
- 375 下实际点击输入框：获得焦点，输入 `qa-check@example.com` 正常显示，宽 285px，无裁切（未提交，未向接口发送）。局部图 `docs/screens/mobile-section-plus.jpg`。
- `pnpm test` 25/25；`pnpm build` 通过；eslint、tsc 通过。桌面回归图 `docs/screens/desktop-section-explore.jpg`、`desktop-full-1440.jpg` 已重新生成。
- 未重新生成路线、未重拍应用源截图（按第 24 节说明不需要）。

### 下一步

等 GPT Work 对两项复验。通过后按第 20 节的运维交付流程打新的源码包（`git archive` 自 `company`）；`main` 是否快进跟随由用户决定。


## 26. GPT Work QA · 2026-09-15（BI-01 / BI-02 复验 · company）

**No blocking issues. 本轮官网设计实现验收通过。**

复验提交：`a48182e`。已读取第 25 节交接，并独立核对相对 `6e708fc` 的实际 diff、最新手机 S05 / S08 局部截图与桌面 S05 回归截图。手机截图按 375 CSS px 宽还原查看，未以 2× 放大图替代正常阅读尺寸。

| 项目 | 复验结果 | 结论 |
|---|---|---|
| BI-01 手机路线不可读 | 官网路线面板已改为手机单列，地图在前、同一路线结果在后。375px 正常尺寸下可辨认地点名、站号及路线摘要；桌面截图仍保持原双列。没有更换路线数据或新增功能。 | 已关闭 |
| BI-02 手机输入框过矮 | 输入框已增加 `min-h-11`，手机 `flex-none`，横排再 `sm:flex-1`。最新截图中的输入框与提交按钮高度一致，文字不再挤边；第 25 节报告 360/375/430 宽实测均为 44 CSS px，并已实际聚焦输入。 | 已关闭 |

代码改动限制在 SiteLanding 两处布局修复；其他改动为截图/尺寸检查脚本和交接文档。邮箱校验、请求与状态逻辑未变；Hero、See、App 业务逻辑与路线源截图未变。未发现本轮修复引入的新 Blocking Issue。

验证边界：本轮由 GPT Work 独立查看代码和实际截图；360/430px 尺寸测量、输入操作与 build/test 结果来自 Claude 第 25 节记录，未冒称重新运行。前轮标明未测的真机 iframe 权限、音频 error 态等仍保持原状态，本次验收不等于这些环境已验证或生产已部署。

**下一步交给 Claude：按第 20/25 节既定流程，从 company 打包最新源码并交运维，附提交号与已有部署说明。不继续开启视觉优化或新功能；main 是否跟随仍按用户决定。本轮 QA 已收口。**
...
## 51. GPT 首屏满屏、四格居中与页内转场需求（2026-09-16）

- 用户明确：同一页面，导航及下滚均定位下方内容区，不切换网址；“详情页”指官网正文，不新增独立页面。
- 交付 `SITE-V2-HERO-TRANSITION-REQUIREMENTS-20260916.md`，涵盖满屏计入导航/署名、四格统一居中、导航目标表、一次性短转场到Moment、中断/防误触/减少动态/无JS、V01–V08验收。
- 明确覆盖旧Hero绝对高度与M-N1相关行为；其余音频/按钮动效和静态地图保持。功能业务验证按用户新范围移到上线后，当前仅验视觉及本次导航/转场交互。
- 当前Hero已有桌面min-height视口计算，尚不能据此认为整体满屏和转场已完成；照片裁切与锚点需随新构图复核。英文与浏览器中文翻译场景均检查四格溢出。
- 本轮仅写需求，未改页面代码、未执行自检、未发布。


## 51. 全站分屏 / 统一宽度 / 四卡重做 / 首屏出口转场（Claude，2026-09-16）

- 触发：用户先在手机上发现导游段整段右溢出被裁、首屏四个信息格文字溢出且不居中，并提出「首屏满铺、顶部导航进入其他内容、下滚有过渡画面再进入详情」。随后 GPT 把这些整理成 `SITE-V2-HERO-TRANSITION-REQUIREMENTS-20260916.md` v2，明确「同一页面内定位、不换网址」，并**覆盖**了此前「按样张固定 Hero 高度」「仅首屏满屏、禁止分屏吸附」「1440px 容器 + 5% 屏边」等旧结论。
- **先修两个真实缺陷**（手机上肉眼可见）：
  - 导游段整段右溢出。根因是 `Reveal` 用 `display: contents`，网格容器上的 `[&>*]:min-w-0` 落在它身上而不是真正的网格项，卡片的最小内容宽把整列撑宽，连左侧标题一起变宽，再被 `.rr2` 的 `overflow-x: clip` 硬裁。已给真正的网格项补 `min-w-0`，并让卡片名字行与播放行在窄屏换行、窄屏肖像收窄。
  - 四个信息格。原本是左图标右文字 + `whitespace-nowrap`，四列网格被最小内容宽撑破左栏、压到照片上且文字被裁。改为「图标在上、主值、说明」整组居中，去掉 nowrap，≥1440 一行四列、其余 2×2，经纬度每行一个完整坐标。
  - 同时修了取证盲区：`.rr2` 是 `overflow-x: clip`，`scrollWidth − innerWidth` 永远为 0，旧的横向溢出检查看不出这类问题；已改为逐元素与裁切祖先求交后再比视口，并补 390/412 两个手机宽度。
- **按 v2 需求实施**：全站统一舞台（90vw / 上限 1800px / 居中，手机固定 20px）；六屏结构（顶栏+Hero 合一屏 → Moment → See & Hear → Explore → Guides → Start+Plus+Footer 合一屏），每屏内容垂直居中；桌面用原生 CSS `scroll-snap`（不写滚动控制器、不吞滚轮），且只在「≥1024 宽、≥640 高、未开减少动态、每一屏都装得下视口」时打开，任何一屏被撑高就整体关闭；导航与 `Scroll to explore` 定位到屏起点（偏差 0px）并给目标屏程序化焦点；手机新增原生 `<details>` 的 Menu 按钮（无 JS 也能用）；Explore 插画收进舞台、保持 4:1 原图、标题左边界与其它屏一致；首屏出口一次性淡入「Discover a deeper layer」提示（180ms 淡入，不占一屏、不控制滚动、点任何锚点即跳过）。
- **自检**：`SITE-V2-HERO-TRANSITION-SELF-CHECK-20260916.md`，证据 `docs/screens/site-v2-hero-transition/2026-09-16T09-19-30/`，V01–V09 全部通过（V02/V03/V04 为先失败后修正）。首屏在 1440×900 / 1280×800 / 1920×1080 与视口差 0px；标题左边界 1440 全为 72px、1920 全为 96px；手机与矮屏 snap 关闭、四格仍在正文之前；减少动态与无 JS 均为完整普通长页。唯一未证实项：锚点原生平滑定位途中的反向手势能否中断，无头环境复现不了（已说明页面没有任何自定义滚动控制器，滚轮未被锁、位置不会被拖回），建议真机复核。
- **连带复测**：动效全套 S1–S7 在新布局重跑全部 PASS（`docs/screens/site-v2-motion/2026-09-16T09-11-54/`）；上线前 P01/P02/P03/P05/P07/P08/P09 重跑全部 PASS（`docs/screens/site-v2-release/2026-09-16T09-15-36/`）。两份旧报告已各加一节「分屏改造后的复测」。
- **遗留**：各内容屏的填充率 25%–50%（Explore 按需求本就不靠拉长地图撑满），要再提高需重做栏目构图，已在自检报告里列出供判断。P04（真机）、P08（联系邮箱）仍 BLOCKED。
- 本轮未部署。基线提交 `85b07bd` 已推到 `origin/company`（Vercel 预览构建成功，但项目开了预览保护，链接需登录或改设置）；分屏这一轮的改动见下一次提交。
