...
| TripAdvisor 等点评 | 没有开放接口，「导游」页放「旅行者观点」付费入口，以后由 AI 检索归纳 | 暂无 |
...
- **帮我规划今天** `/api/plan` + `lib/plan.ts`：全靠按钮，先选时间（1 小时 / 半天 / 一天），再多选兴趣。模型只负责「从候选里挑几站并说为什么」，其余由代码兜底：
  ...
  结果存 localStorage（`lib/routeStore.ts`）。导游页整段对话也存 localStorage（`tourguide.guideChat`，一天有效，头部「新对话」清掉），切 tab 或刷新不丢；地点对话按地点存 sessionStorage（`tourguide.talk:<lang>:<id>`），关掉应用即清，「在地图上看」回到首页：编号图钉 + 虚线 + 顶部路线栏 + 底部当前站卡片（耳机图标直达讲解），叉掉即结束。
- **拍照翻译**：相机弹层多了「这是什么 / 翻译文字」切换，`/api/identify` 加 `mode=translate`，读出原文并翻成讲解语言。
- **旅行者观点、主题漫步、旅行日记**：只有入口，标「付费功能」，点了导游用一句话说明这个功能将来做什么并告知尚未开放。「旅行者观点」= AI 读旅行论坛和点评网站上去过的人的真实评价并归纳（原「旅行者论坛观点」，从详情页挪到这里）。
