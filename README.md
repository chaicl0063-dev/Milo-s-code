...
- 首页 = 地图 + 可拖动的底部面板（收起 / 半屏 / 接近全屏三档）。「浏览中心」和「我的位置」分开：搜索围着前者，蓝点画在后者。顶部胶囊显示浏览中心的地名（Nominatim 反向地理编码），点它换城市；地图拖远后出现「搜索这一片」；右下按钮回到我的位置。
...
- **朗读**：浏览器自带的 `speechSynthesis`，免费离线。`lib/useNarrator.ts` 把讲解按句切开逐句读（能边流式边读，也绕开 Chrome 长段落中断的问题），当前句高亮；设置里有「自动朗读」开关。音色取决于手机系统装的语音包。
